...
void fb155bc_task_init(void)
{
   // battery_init();
}

void fb155bc_task(void)
{
    /*int16_t c;
    // Raw mode: interpret output report of LED state
    while ((c = fb155bc_getc()) != -1) {
    ...
    if (!config_mode && !force_usb) {
        if (!fb155bc_rts() && host_get_driver() != &fb155bc_driver) {
            clear_keyboard();
            host_set_driver(&fb155bc_driver);
        } else if (fb155bc_rts() && host_get_driver() != &lufa_driver) {
            clear_keyboard();
 ...
 * Command
 ******************************************************************************/
static host_driver_t *prev_driver = &fb155bc_driver;

static void print_fb155bc(void)
{
    int16_t c;
    while ((c = fb155bc_getc()) != -1) {
        xprintf("%c", c);
...
static void clear_fb155bc(void)
{
    while (fb155bc_getc() != -1) ;
}
...
static void enter_command_mode(void)
{
    ...
    clear_keyboard();
    host_set_driver(&fb155bc_config_driver);   // null driver; not to send a key to host
    fb155bc_disconnect();
    //while (fb155bc_linked()) ;
...
static void init_fb155bc(void)
{
    // FB155BC-HID configure
...
bool command_extra(uint8_t code)
{
    ...
    switch (code) {
        case KC_H:
        case KC_SLASH: /* ? */
            print("\n\n----- Bluetooth FB155BC-HID Help -----\n");
            print("i:       FB155BC-HID info\n");
            print("b:       battery voltage\n");
            print("Del:     enter/exit FB155BC-HID config mode\n");
            print("Slck:    FB155BC-HID initialize\n");
#if 0
        ...
        case KC_I:
            print("\n----- FB155BC-HID info -----\n");
            xprintf("protocol: %s\n", (host_get_driver() == &fb155bc_driver) ? "FB155BC-HID" : "LUFA");
            xprintf("force_usb: %X\n", force_usb);
            xprintf("fb155bc: %s\n", fb155bc_rts() ? "OFF" : (fb155bc_linked() ? "CONN" : "ON"));
            xprintf("fb155bc_autoconnecting(): %X\n", fb155bc_autoconnecting());
        ...
        case KC_U:
            ...
            return true;
        case KC_DELETE:
            /* FB155BC-HID Command mode */
            if (fb155bc_autoconnecting()) {
                enter_command_mode();
